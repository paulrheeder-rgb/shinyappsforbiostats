# ============================================================
# Descriptive Statistics Summary App (with download options)
# ============================================================
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      h4("Save Cleaned Dataset"),
      downloadButton("download_data", "💾 Download Cleaned Data (.RData)", class = "btn-info")
    ),
...
server <- function(input, output, session) {
  ...
  # ---- Download Word file for summary ----
  ...
  # ---- Download cleaned dataset as RData ----
  # ---- Download cleaned dataset as RData ----
  output$download_data <- downloadHandler(
    filename = function() {
      paste0("cleaned_data_", Sys.Date(), ".RData")
    },
    content = function(file) {
      req(filtered_data())
      # Save the reactive dataset object into the RData file
      saveRDS(filtered_data(), file = file)   # alternative: save(list = "filtered_data", file = file)
    }
  )
}
# ---- Launch ----
